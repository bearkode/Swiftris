...
extension Grid {
    ...
    internal subscript(position: Point) -> T? {
        get {
            assert(self.size.validate(point: position), "Index out of range")
            return self.buffer[self.size.index(of: position)]
        }
        set {
            assert(self.size.validate(point: position), "Index out of range")
            self.buffer[self.size.index(of: position)] = newValue
        }
    }
